...
//  Random-User-List
//
//  Created by iMaxiOS on 10/11/18.
...
public extension UITextField {
    ...
    //Validate a phone for the right format
    func isValidPhone(phone: String?) -> Bool {
        let PHONE_REGEX = "^[0-9-+s()]*$"
        let pred = NSPredicate(format:"SELF MATCHES %@", PHONE_REGEX)
        return pred.evaluate(with: phone)
    }
    
    //Validate a name for the right format
